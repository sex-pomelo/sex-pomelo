...
starter.setEnv = function( runEnv ){
    env = runEnv;
};
 ...
 *
 */
starter.sshrun = function(cmd, host, cb,opts) {
  let args = [];
  args.push(host);
  ...
  spawnProcess(Constants.COMMAND.SSH, host, args, cb,opts);
  return;
};
 ...
 *
 */
starter.localrun = function (cmd, host, options, callback,opts) {
  logger.info('Executing ' + cmd + ' ' + options + ' locally');
  spawnProcess(cmd, host, options, callback,opts);
};
 ...
 *
 */
let spawnProcess = function(command, host, options, cb,opts) {
  let child = null;
  if(env === Constants.RESERVED.ENV_DEV) {
    let spawnOpt = {};
    if( opts.daemon ){
      spawnOpt = {detached: true, stdio: 'ignore'};
    }
    child = cp.spawn(command, options,spawnOpt);
    if( opts.daemon !== true){
