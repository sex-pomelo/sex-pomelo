...
Monitor.prototype.start = function(cb) {
  ...
  this.client.on('close', function() {
    logger.warn(`[redisMonitor] server ${self.app.getServerId()} has been closed with redis server.`);
  });

  this.client.on('end', function() {
     logger.warn(`[redisMonitor] server ${self.app.getServerId()} is over and without reconnection.`);
  });
};
...
var updateServerInfo = function(self, app, redis, serverInfo) {
  var key = g_preRegSer + app.env + ":" +serverInfo.id;
  serverInfo.pid = process.pid;
...
var getCommand = function(self, app, redis, serverInfo) {
  var key = g_preReg + app.env + ':' + serverInfo.id;
